...
class hdatmos():
    ...
    def update_cookies(self, cookies):
        self.my_session.cookies.update(
            dict([line.strip().split('=', 1) for line in cookies.split("; ")]))
    ...
    def attendance(self):
        ...
        if(response.text.find('签到成功') != -1):
            msg = response.text[response.text.find('<td class="text" >')+19:response.text.find(
                '<span style="float:right">今日签到排名：<b>')].replace('</b>', '').replace('<b>', '').replace('<p>', '').split('。')[0]
            success = True
        else:
            msg = '签到失败，请手动完成签到'
            success = False
